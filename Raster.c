...
static int16_t	Set_Road_Height(void)
{
	...
	/* コースデータ読み込み */
	road_height = (int16_t)(0x80 - g_stRoadData[g_uCounter].bHeight);	/* 道の標高	(0x80センター) */
...
//				G_Load_Mem( ubPatNumber, uWidth * i, Y_MIN_DRAW + Y_OFFSET + Y_HORIZON_1 - uHeight + RASTER_MIN, 1);	/* 背景(下側) */
			}
		}
		else
		{
