...
int16_t RasterVcyncProc(void);
int16_t RasterProc(uint16_t *);
...
const uint16_t D_RAS_INT_MAX[2]	= { 512 + 32,	 	Y_MAX_WINDOW + 16	};	/* 0:31kHz 1:15kHz */
...
//	ViewAngle = - Diff;	/* -128 ot 0 to 128 */
	...
	for(uRas_y=0; uRas_y < uRas_size; uRas_y++ )
	{
		...
		stRasterInt.x = M512Roll((uint16_t)ras_cal_x);
		stRasterInt.y = M512Roll((uint16_t)ras_cal_y);
//		stRasterInt.x = ras_cal_x;
//		stRasterInt.y = ras_cal_y;
...
static int16_t Raster_SetBndngCnd(int16_t *x, int16_t Count, int16_t Size)
{
	...
	{
		*x = (int16_t)APL_sDiv(360, (angle * Count + 1) );
	}
...
//	Road_strch = (Num * Num)/200 - 64;	/* 二次曲線 */
	...
	Point_y = M512Roll((uint16_t)(Point_y_def + RoadPoint_y));	/* 0 - 512 */
	*y = Point_y;
...
//				y = Y_MIN_DRAW +        0 + Y_HORIZON_1 - uHeight + RASTER_MIN;
				y = Y_MIN_DRAW;
				ret = G_BitBlt_From_Mem( x, y, 1, pSrcBuf, uWidth, uHeight, 0, POS_LEFT, POS_TOP, ubPatNumber);	/* 背景(上側) */
				y = Y_MIN_DRAW + Y_OFFSET;
				ret = G_BitBlt_From_Mem( x, y, 1, pSrcBuf, uWidth, uHeight, 0, POS_LEFT, POS_TOP, ubPatNumber);	/* 背景(下側) */
...
{
	int16_t ret = 0;
	uint16_t	uRasterPos = 0;
	
	if( GetRasterCount(&uRasterPos) == TRUE )	/* ラスタ割り込み可能 */
	{
		uint16_t	x, y;
		uint16_t	uSide;
		uint16_t	uCRT_Tmg;
		...
		*VIDEO_REG3 = Mbset(*VIDEO_REG3,   0x0C, 0b0000000000001100);	/* GR2(GR3,GR4)=ON */
		...
		uRas = g_stRasterInt[(RASTER_H_MAX - 2) + g_uRasterSideNow].y + SP_Y_OFFSET;
		...
		/* ラスタ */
		SetRasterCount( uRas, D_RAS_INT_MAX[uCRT_Tmg]);
...
/* 機能		：	*/
/*===========================================================================================*/
int16_t RasterProc(uint16_t *pRaster_cnt)
{
	int16_t ret = 0;
	uint16_t	uRas_Count;
	static uint16_t	s_uRas;
	
	uRas_Count = *pRaster_cnt;			/* 配列番号(0)＝ラスタ割り込み開始位置 */
	*pRaster_cnt += g_uRas_NexrCount;	/* 次のラスタ割り込み位置の計算 */
	...
	else
	{
		...
		/* BGの処理 */
		GetRasterIntPos( &x, &y, &pat, s_uRas, FALSE);	/* 配列位置からデータを取得 *//* g_uRas_NexrCountが２だから２倍してる */
		/* BG0 */
		*BG0scroll_x	= x;		/* BG0のX座標の設定 */
		*BG0scroll_y	= y + pat;	/* BG0のY座標の設定 */
		/* BG1 */
		*BG1scroll_x	= x;		/* BG1のX座標の設定 */
		*BG1scroll_y	= y + pat;	/* BG1のY座標の設定 */
	}
	s_uRas += RASTER_NEXT;	/* 次の配列 */
