...
## やりなおし
* HELPキーを長押しで強制ゲームオーバー
  
## レジューム機能（現状のボールの配置を保存）   (ver0.1.2)
* F6キーを長押し
HARDモード選択時、レジュームした時の状態で再開できる。  

## オリジナルのテーマを作ろう
１．dataディレクトリ内に存在する_default_ディレクトリを複製します。  
　　（例：複製したディレクトリ名をmoemoeとする）  
...
## ダウンロード  
[[Download]](https://github.com/kata68k/X68000/blob/master/Game/SuiKata/SUIKATA012.LZH)	ver0.1.2(2024/10/07)  

## やることリスト  
*  処理負荷改善 10MHz機/16MHz機/24MHz機  
*  巨大はボタンが押されるまで表示を継続  
*  難易度が上昇する仕組み  
*  MACS動画 再生演出  
*  玉を当てて動かしたい
*  減速処理が思ったとおりに動いてない
*  大玉消去時の余韻がほしい
*  一度、ボーダーラインを超えないとピンチ状態にならない (ver0.1.3)  
*  消去時のテキストパレットの整理 (ver0.1.3)  
*  シングルメニューなのに選択時の挙動が出る (ver0.1.3)  
*  端っこで発射すると常時連射になる (ver0.1.3)  

## 履歴  
* 2024/10/07	ver.0.1.2  
** 起動メニューのジョイスティック対応  
** 当たり判定を小さくした  
** 合体時のリアクションを追加  
** スプライト情報とボールの情報が乖離しているので修正  
